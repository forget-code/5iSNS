...
function db_query($sql, $d = NULL) {
	
	$db = $_SERVER['db'];
	$d = $d ? $d : $db;
	if(!$d) return FALSE;
	
	DEBUG AND xn_log($sql, 'db_query');
...
function db_sum($table, $field, $cond = array(), $d = NULL) {
	$db = $_SERVER['db'];
	$d = $d ? $d : $db;
	if(!$d) return FALSE;
	
	$r = $d->sum($d->tablepre.$table, $field, $cond);
...
function db_minid($table, $field, $cond = array(), $d = NULL) {
	$db = $_SERVER['db'];
	$d = $d ? $d : $db;
	if(!$d) return FALSE;
	
	$r = $d->minid($d->tablepre.$table, $field, $cond);
...
function db_find_column($table, $cond = array(), $key = '', $d = NULL) {
	$db = $_SERVER['db'];
	...
	return $d->find_column($table, $cond, $key);
}
function db_find_all($table, $cond = array(), $orderby = array(), $key = '', $col = array(), $d = NULL) {
	$db = $_SERVER['db'];
	...
	return $d->find_all($table, $cond, $orderby, $key, $col);
}
...
function db_concat_field($field_arr){
	$s = '';
